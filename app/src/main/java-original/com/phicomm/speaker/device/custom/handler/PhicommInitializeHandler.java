...
public class PhicommInitializeHandler extends ANTEventDispatcher implements PhicommDeviceStatusProcessor.OnDeviceStatusChangedListener {
    ...
    private void playInitDoneTips() {
        ...
        if (!NetUtil.isNetworkConnected(this.mContext)) {
            this.isFristBoot = true;
            if (UserPerferenceUtil.getDeviceBindState(this.mContext)) {
                this.mANTEngine.playTTS(this.mContext.getString(R.string.tts_net_is_weak));
            } else {
                this.mANTEngine.playTTS(this.mContext.getString(R.string.tts_bootloader_welcome));
            }
        } else {
            initPhicommBusiness();
        }
