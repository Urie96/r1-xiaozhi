...
public class PinyinConverter {
    ...
    */
    private static String transWord2Pinyin(Character cn2, boolean needMulti) {
        byte[] bytes;
        String str;
        String string;
        int uIntFrombytes;
        String pinYinbyIndex = "";
        Character cn3 = Character.valueOf(transT2S(cn2.charValue()));
        try {
            bytes = String.valueOf(cn3).getBytes("UTF-16LE");
        } catch (UnsupportedEncodingException e) {
            bytes = null;
        }
        if (bytes == null || bytes.length <= 0 || (uIntFrombytes = getUIntFrombytes(bytes, 0, bytes.length)) < MANDPY_START || uIntFrombytes > MANDPY_END) {
            str = pinYinbyIndex;
            string = "";
        } else {
            int i = (uIntFrombytes - 19968) * 2;
            if (mUnit2PinyinIndex != null && mUnit2PinyinIndex.length > i + 2) {
                int intFrombytes = getIntFrombytes(mUnit2PinyinIndex, i, 2);
                if (intFrombytes != 9999 && intFrombytes >= 0) {
                    pinYinbyIndex = getPinYinbyIndex(intFrombytes);
                }
                if (intFrombytes < 0) {
                    int i2 = (20902 - intFrombytes) * 2;
                    int intFrombytes2 = getIntFrombytes(mUnit2PinyinIndex, i2, 2);
                    int[] iArr = new int[-intFrombytes2];
                    StringBuilder sb = new StringBuilder();
                    int i3 = i2 + 2;
                    str = null;
                    for (int i4 = 0; i4 < (-intFrombytes2); i4++) {
                        iArr[i4] = getIntFrombytes(mUnit2PinyinIndex, i3, 2);
                        String pinYinbyIndex2 = getPinYinbyIndex(iArr[i4]);
                        if (str == null) {
                            str = pinYinbyIndex2;
                        }
                        if (sb.length() <= 0) {
                            sb.append(pinYinbyIndex2);
                        } else {
                            sb.append(PINYIN_SEPARATOR);
                            sb.append(pinYinbyIndex2);
                        }
        ...
        if (mUncommonWordsMap.containsKey(cn3)) {
            str = mUncommonWordsMap.get(cn3);
        }
        return (!needMulti || string.equals("") || string.equals(str)) ? str : string;
    }
}
