        ...
        try {
            if (typeTopByte == 169 || typeTopByte == 65533) {
                ...
                if (shortType == SHORT_TYPE_COMMENT) {
                    textAttribute = parseCommentAttribute(type, ilst);
                } else if (shortType == SHORT_TYPE_NAME_1 || shortType == SHORT_TYPE_NAME_2) {
                    textAttribute = parseTextAttribute(type, "TIT2", ilst);
                    ilst.setPosition(endPosition);
                } else if (shortType == SHORT_TYPE_COMPOSER_1 || shortType == SHORT_TYPE_COMPOSER_2) {
                    textAttribute = parseTextAttribute(type, "TCOM", ilst);
                    ilst.setPosition(endPosition);
                } else if (shortType == SHORT_TYPE_YEAR) {
                    textAttribute = parseTextAttribute(type, "TDRC", ilst);
                    ilst.setPosition(endPosition);
                } else if (shortType == SHORT_TYPE_ARTIST) {
                    textAttribute = parseTextAttribute(type, "TPE1", ilst);
                    ilst.setPosition(endPosition);
                } else if (shortType == SHORT_TYPE_ENCODER) {
                    textAttribute = parseTextAttribute(type, "TSSE", ilst);
                    ilst.setPosition(endPosition);
                } else if (shortType == SHORT_TYPE_ALBUM) {
                    textAttribute = parseTextAttribute(type, "TALB", ilst);
                    ilst.setPosition(endPosition);
                } else if (shortType == SHORT_TYPE_LYRICS) {
                    textAttribute = parseTextAttribute(type, "USLT", ilst);
                    ilst.setPosition(endPosition);
                } else if (shortType == SHORT_TYPE_GENRE) {
                    textAttribute = parseTextAttribute(type, "TCON", ilst);
                    ilst.setPosition(endPosition);
                } else if (shortType == TYPE_GROUPING) {
                    textAttribute = parseTextAttribute(type, "TIT1", ilst);
                    ilst.setPosition(endPosition);
                } else {
                ...
                textAttribute = parseStandardGenreAttribute(ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_DISK_NUMBER) {
                textAttribute = parseIndexAndCountAttribute(type, "TPOS", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_TRACK_NUMBER) {
                textAttribute = parseIndexAndCountAttribute(type, "TRCK", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_TEMPO) {
                textAttribute = parseUint8Attribute(type, "TBPM", ilst, true, false);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_COMPILATION) {
                textAttribute = parseUint8Attribute(type, "TCMP", ilst, true, true);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_COVER_ART) {
                textAttribute = parseCoverArt(ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_ALBUM_ARTIST) {
                textAttribute = parseTextAttribute(type, "TPE2", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_SORT_TRACK_NAME) {
                textAttribute = parseTextAttribute(type, "TSOT", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_SORT_ALBUM) {
                textAttribute = parseTextAttribute(type, "TSO2", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_SORT_ARTIST) {
                textAttribute = parseTextAttribute(type, "TSOA", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_SORT_ALBUM_ARTIST) {
                textAttribute = parseTextAttribute(type, "TSOP", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_SORT_COMPOSER) {
                textAttribute = parseTextAttribute(type, "TSOC", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_RATING) {
                textAttribute = parseUint8Attribute(type, "ITUNESADVISORY", ilst, false, false);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_GAPLESS_ALBUM) {
                textAttribute = parseUint8Attribute(type, "ITUNESGAPLESS", ilst, false, true);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_TV_SORT_SHOW) {
                textAttribute = parseTextAttribute(type, "TVSHOWSORT", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_TV_SHOW) {
                textAttribute = parseTextAttribute(type, "TVSHOW", ilst);
                ilst.setPosition(endPosition);
            } else if (type == TYPE_INTERNAL) {
                textAttribute = parseInternalAttribute(ilst, endPosition);
                ilst.setPosition(endPosition);
            }
