...
public class ChunkSampleStream<T extends ChunkSource> implements SampleStream, SequenceableLoader, Loader.Callback<Chunk> {
    ...
    */
    public void seekToUs(long positionUs) {
        ...
        this.lastSeekPositionUs = positionUs;
        if (!isPendingReset()) {
            seekInsideBuffer = this.primarySampleQueue.skipToKeyframeBefore(positionUs, (positionUs > getNextLoadPositionUs() ? 1 : (positionUs == getNextLoadPositionUs() ? 0 : -1)) < 0);
        }
        if (seekInsideBuffer) {
            while (this.mediaChunks.size() > 1 && this.mediaChunks.get(1).getFirstSampleIndex(0) <= this.primarySampleQueue.getReadIndex()) {
            ...
            while (i < length) {
                DefaultTrackOutput embeddedSampleQueue = defaultTrackOutputArr[i];
        ...
        DefaultTrackOutput[] defaultTrackOutputArr2 = this.embeddedSampleQueues;
        int length2 = defaultTrackOutputArr2.length;
        while (i < length2) {
            DefaultTrackOutput embeddedSampleQueue2 = defaultTrackOutputArr2[i];
            embeddedSampleQueue2.reset(true);
            i++;
        }
    }
