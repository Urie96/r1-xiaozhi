...
public final class AudioTrack {
    ...
    public void configure(String mimeType, int channelCount, int sampleRate, int pcmEncoding, int specifiedBufferSize, int[] outputChannels) throws ConfigurationException {
        ...
        if (flush || !isInitialized() || this.encoding != encoding || this.sampleRate != sampleRate || this.channelConfig != channelConfig) {
            ...
            if (specifiedBufferSize != 0) {
                ...
                int maxAppBufferSize = (int) Math.max(minBufferSize, durationUsToFrames(MAX_BUFFER_DURATION_US) * ((long) this.outputPcmFrameSize));
    ...
    private boolean writeBuffer(ByteBuffer buffer, long avSyncPresentationTimeUs) throws WriteException {
        ...
        int bytesWritten = 0;
        if (Util.SDK_INT < 21) {
            int bytesPending = (int) (this.writtenPcmBytes - (this.audioTrackUtil.getPlaybackHeadPosition() * ((long) this.outputPcmFrameSize)));
    ...
    private long applySpeedup(long positionUs) {
        ...
        return this.playbackParametersOffsetUs + ((long) (((double) this.playbackParameters.speed) * (positionUs - this.playbackParametersPositionUs)));
    }
    ...
    private static class AudioTrackUtil {
        ...
        public long getPlaybackHeadPosition() {
            if (this.stopTimestampUs != C.TIME_UNSET) {
                ...
                long framesSinceStop = (((long) this.sampleRate) * elapsedTimeSinceStopUs) / C.MICROS_PER_SECOND;
