...
public final class BaseSessionLayer implements UpDownMessageManager.MessageExcuteListener {
    ...
    public void filterSessionContent(String message) {
        ...
        if (!TextUtils.isEmpty(message)) {
            ...
            SessionData<JsonObject> messageBody = cloudResponse.getMessageBody();
            if (messageBody == null) {
            ...
            if (CloudResponse.MESSAGE_TYPE_INTENT_ACTION.equals(messageType) || "synInfo".equals(messageType)) {
                try {
                    String action = messageBody.getDstService().getUniCommand().getOperation();
                    DialogProfile dialog = messageBody.getDialog();
                    if (dialog == null) {
                ...
                dispatchControlSessionMsg(messageBody.getDialog().getDstService(), messageBody.getDstService());
                return;
            }
            if (CloudResponse.MESSAGE_TYPE_UPDATE_REPONSE.equals(messageType) || CloudResponse.MESSAGE_TYPE_UPLOAD_PD_REPONSE.equals(messageType)) {
                receiveCloudResponseSessionMsg(messageBody.getDialog().getDstService(), messageBody.getActionResponse());
            } else if (CloudResponse.MESSAGE_TYPE_START_SYN.equals(messageType)) {
                LogMgr.d(TAG, "cloud command start syn info");
                dispatchSyncServiceStatusReq(messageBody.getDialog().getDstService());
            }
        }
    }
