...
public class LocalizeDataUtil {
    ...
    public static CommandInfo getCommandInfo(String wakeupWord) {
        ...
        for (CommandInfo commandInfo : mCommandInFile) {
            if (commandInfo.getCommand().contains(wakeupWord)) {
                return commandInfo;
            }
        }
        return null;
    ...
    */
    public UniContact getContactByPhoneNumber(String phoneNumber) {
        if (this.mContacts.size() == 0) {
            updateContacts();
        }
        for (UniContact uniContact : this.mContacts) {
            ArrayList<String> contactPhoneNO = uniContact.getContactPhoneNO();
            if (contactPhoneNO.size() > 0) {
                int i = 0;
                while (true) {
                    int i2 = i;
                    if (i2 < contactPhoneNO.size()) {
                        if (phoneNumber.equals(contactPhoneNO.get(i2).replaceAll(PinyinConverter.PINYIN_SEPARATOR, ""))) {
                            return uniContact;
                        }
                        i = i2 + 1;
                    }
    ...
    public List<String> updateUserCommands() throws Throwable {
        ...
        try {
            ...
            for (CommandInfo commandInfo : mCommandInFile) {
                ...
                LogMgr.i("like", "mCommand = " + command + " mOperands= " + commandInfo.getOperands() + "  mOperator=" + commandInfo.getOperator() + " mValue=" + commandInfo.getValue());
                if (!TextUtils.isEmpty(command)) {
                    ...
                    for (int i = 0; i < strArrSplit.length; i++) {
                        ...
                        if (!TextUtils.isEmpty(strArrSplit[i])) {
                            ...
                            this.mCommandMap.put(strArrSplit[i].trim(), commandInfo);
                        }
                    }
                }
            }
